...
<script type="text/javascript" >
		...
		xhttp.onreadystatechange = function() {
			if(xhttp.readyState == 4 && xhttp.status == 200){		// 정상상태일때 한번만 실행시키게 해준다.
				if (xhttp.responseText){							// 응답이 왔으면 실행한다. 이 두개의 if가 없으면 여러번 실행된다.
					...
					temp = temp.replaceAll(']','');
					episode = temp.split(',');
					makeSelection(episode,event);					// 만들어진 에피소드 array를 보내 select 함수를 만든다.
				}
			}
		};
		xhttp.send();
	...
	function makeSelection(data,event){							// 에피소드를 받아서 셀렉트(드롭박스)를 만드는 함수.
		...
		for(var j=0; j<data.length; j++){						// 받아온 에피소드 수 만큼 반복해주면서 option을 생성한다.
			...
			option.value = data[j];								// option 값을 넣어준다.
			option.text = data[j];								// 표시될 옵션 텍스트를 넣어준다.
			select.appendChild(option);							// 만들어진 옵션을 셀렉트 밑에 넣어준다.
		}
		console.log(event);		
		/* selDiv.parentNode.removeChild(select); */
		document.getElementById(event).appendChild(select);		// 해당 사진 밑에있는 div에 만들어진 셀렉트를 넣어준다.
	}
